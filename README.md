...
Where a more prescriptive framework gives the operator a workflow to drive, SkipHow states a contract and leaves the sequence to the agent. Treat that as a design position rather than a measured comparison; nothing here benchmarks one against the other. What is measured, on one host and one small project, is that a reported bug fix opened two method files and finished in about a minute, while a new feature stopped to ask which product it should be. The runs are in [current evidence](docs/evidence.md).
