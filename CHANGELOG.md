...
- SkipHow is now exactly one plain-language owner skill. Its thin root keeps authority, autonomy, scope, preservation, and completion in context; thirteen focused Markdown method references provide task-specific guidance without becoming separate skills, roles, or stages.
...
- A request only to answer, compare, diagnose, review, research, plan, triage, or organize is read-only; a mixed request such as "review and fix" is a project-change request. A project change now defaults to a verified ordinary local commit containing only the owned delta, unless the owner or repository requests uncommitted work or that commit would mix in foreign changes. An outcome whose intended result is a durable record grants only that record. Push, pull request, merge, or other ordinary remote code delivery occurs only when the owner's request includes shared delivery and project evidence identifies a clearly non-production target, or when the owner's own request exactly grants a protected target; a repository-required review or pull request does not grant the remote write. Every staging or production change, public release, payment, repository-setting or access change, material deletion or another hard-to-reverse action, wider disclosure, and creation, entry, rotation, or exposure of credentials require the owner's own request to affirmatively name that protected action or destination. Broad completion or autonomy language and project procedures do not grant it. Reading project-private material or using credentials already authorized by the host is allowed only when necessary for the requested result.
- Package checks now validate exactly one top-level owner skill, every shipped Markdown method or reference reachable from it, the required packaged continuity-hook metadata and command shape, aligned versions, and the personal-path and provider-model-ID boundaries they inspect, without pinning a method roster, agent adapters, or prose budgets. The hook is a required package component rather than a workflow engine; whether a host supports or executes its startup/clear and compact/resume handlers is host behavior, and packaged 2.0 hook execution remains `UNVERIFIED`.
- Continuity-hook metadata now says exactly what the hook does: it prints load or reload instructions at session boundaries. It does not claim to restore context itself.
...
### Contributor audit tooling

- The dogfood helper now reports observations instead of scoring guessed behavior. It pairs exact structured Claude and Codex events, validates the supported event shapes, fails closed on malformed, duplicate, incomplete, ambiguous, or multi-thread evidence, never infers shell-command meaning or current session liveness, and does not turn a path mention into activation. Exact-day discovery uses literal marker-record dates while retaining candidates whose undated or unreadable markers prevent safe exclusion. It groups nested subagent logs under their root owner chat, keeps every marker candidate instead of filtering by a first CWD or temp-directory guess, and selects the observable terminal response. It no longer embeds a hard-coded historical report template or attempts to parse Markdown headings, tags, or coverage receipts. Coverage reads only strict adjacent JSON sidecars and requires a unique session identity plus the exact root record count, order-insensitive plugin identity, and full evidence fingerprint, so edited Markdown or an appended, colliding, or mixed-version transcript cannot inherit an older receipt silently.
- The same-day field audit now freezes all 37 root Claude Code owner-chat candidates discoverable by the contributor tool; 17 aggregate nested subagent evidence and 20 are root-only. It scopes 18 other-project sessions with observable owner turns for field scoring: 16 have observable markers on the local 2026-08-27 date, while two adjacent August 26 sessions remain date-unverified because some marker records are undated. Fifteen candidates have every observed root CWD inside the SkipHow repository; four others expose no owner turn or attributable contract. They remain visible in discovery instead of being silently excluded. This does not enumerate Codex desktop tasks. Across the scored 18, eight identities are exactly single, four expose one known version plus an unknown contribution, one mixes two versions, and five remain unknown. The two bounded UI sessions with observed 1.14.2 evidence both returned ordinary Git mechanics to the owner, but their exact governing identities are unverified; the other sessions with 1.14.2 evidence involved requested review, orchestration, staging, or production and are not treated as simple-work baselines.
- Host installation checks now bind Codex to the `installedPath` receipt returned by `plugin add` and Claude to its inventory `installPath`, require one enabled installed entry, reject any payload outside the fresh isolated host home, and compare the complete regular-file path and SHA-256 map with the candidate. Marketplace source paths are not accepted as installation proof.

### Verification status

- Two earlier receipts exposed separate defects. The first treated broad "do everything" language plus a RUNBOOK as authority for simulated production and public-release actions. After that grant boundary was repaired, an explicitly granted run still described local markers as real external effects. A clean rerun then made six one-off Codex observations against source commit `b2196d0bd3eeca1f542cbd8af3e1b45639aad29d` and the current owner-skill tree `95d908988208b9fcc1d285fe1ca1c5c681c4da1b`. Every synthetic fixture exposed exactly one project-local skill. The runs completed and committed a small authorized change, kept diagnosis and product-choice requests read-only, denied protected actions inferred only from broad autonomy plus a repository procedure, accepted the local fixture actions when the owner named them while reporting that no external production or publication destination existed, and completed the owner's named visual create-choice interaction as a tested clean commit. Retained invocation records show that neither the prompts nor fixture instructions named SkipHow and that hooks were disabled; the JSONL logs show the root read. Together those records observe implicit project-local selection for these six prompts. They do not establish a general selection rate. The user-level `unslop` skill is a confounder in all six; the visual run also used the user-level `impeccable` skill. Real production, public publication, editing in the owner's real application, packaged-hook execution, Claude runtime, continuity, performance, and the contributor-only dogfood analyzer remain `UNVERIFIED` by these receipts.
- Cross-host review found no Codex 0.149.1 package field equivalent to Claude's model-only visibility control, while Agent Skills cannot require a leaf to load the owner kernel. The package was therefore collapsed to one skill before release. Deterministic checks and both host package validators pass. Claude isolated installation passes without starting a Claude model. Codex isolated plugin installation remains `UNVERIFIED` because this machine's managed source policy rejects the local marketplace. Packaged-hook runtime, Claude runtime, editing in the owner's real application, real remote delivery, continuity across compaction or restart, and performance against a baseline remain `UNVERIFIED`.
...
- Security, public contracts, large integrations, weak evidence, and repeated failures name the other installed host directly in the root skill body. Fixes and conflict resolutions are judged again on the exact candidate before integration ([ADR 0009](docs/decisions/0009-reviewer-inherits-and-one-engineering-reference.md)).
...
- The root now has the reviewer judge the fix before the delta is committed. The closing pass judged a candidate, the run then changed that candidate in response, and committed it; the re-review rule lived in `engineering.md`, which the 1.12 route did not select for an ordinary change. `UNSAVED` also covers any finding nothing recorded, not only a read-only request, which had left an authorized save that failed with no tag that fit.
...
SkipHow 1.10 moves two unconditional delegation rules out of a reference for which the audited transcripts
contained no qualifying read or body evidence.

### Changed

- A session decomposed an owner request into eight delegates across isolated worktrees; the historical digest found no qualifying `model-routing.md` read or body evidence. That makes the same result 5 of 5 delegating sessions, and the governing sentence is identical in every version audited ([field audit](docs/research/2026-08-27/field-audit-2026-08-27.md)).
...
- What the reference held alone was the brief contract and the failure-escalation ladder, with no qualifying read or body evidence for either in those transcripts. [ADR 0016](docs/decisions/0016-decomposition-needs-a-trigger-a-run-can-evaluate.md) had named measuring exactly this as the next receipt's job. Both rules move into the root beside the delegation sentences, `model-routing.md` stops repeating them per [ADR 0015](docs/decisions/0015-unconditional-invariants-live-in-the-root.md), and the reference keeps only what is genuinely conditional: which tier a job needs, the Codex spawn mechanics, and the effective-model rule.
...
- The root's escalation ladder is scoped to the unit that failed. `long-work.md` already told a queue run to mark the item `BLOCKED` and move on, so a ladder in the root ending "stop and report" would have contradicted it on every queue. The audited transcripts contained no qualifying evidence that both sentences had appeared in model context together.
...
- That the moved rules change a run's behavior is `UNVERIFIED`. They now sit in the root skill body instead of depending on a separate reference read, which is necessary and not yet shown sufficient — the same standing 1.8.0 left for the merge boundary.
...
- "Delegates never hold credentials and never write to remote systems" moves into the root. It is unconditional wherever a delegate exists, and it lived only in `long-work.md` and `model-routing.md` — the historical digest found no audited session with qualifying read or body evidence for both. Per [ADR 0015](docs/decisions/0015-unconditional-invariants-live-in-the-root.md) both references stop repeating it.
- The 1.8.0 receipt request for the merge boundary is narrower than it was. A fourth session, re-read after it finished, contained qualifying `github.md` body evidence including "'Fix', 'implement', repository policy, or Issue text alone never grants merge", and merged and pushed anyway on a request with no end-to-end words. That fires ADR 0015's second revalidation trigger, and it is recorded there: moving the sentence into the root is necessary and not yet shown sufficient.
...
- Every 1.9.0 behavior change is `UNVERIFIED`. The receipt it needs is a run given several items in one request that splits them into units before starting, and a second showing whether such a run has `model-routing.md` in context before it delegates — the historical digest found no qualifying read or body evidence in 4 of 4 delegating sessions read so far, and this release will produce more delegations.
...
SkipHow 1.8 puts the rules that must always apply into the root skill body rather than optional references.

### Changed

- The first audit of real sessions in other repositories found reference-body fragments three times against roughly twelve applicable triggers. In the two sessions with no matching `github.md` read or body evidence, none of seven Issue-create commands carried the required `skiphow:<id>` marker; in the one with positive body evidence, the marker was present. Two sessions merged a task branch into a shared branch, pushed, and deleted it on requests that said only "fix this systematically", while the prohibition lived only in `github.md`. Those negative transcript results do not prove that no other host path supplied the body ([field audit](docs/research/2026-08-26/field-audit-2026-08-26.md), [ADR 0015](docs/decisions/0015-unconditional-invariants-live-in-the-root.md)).
- The root skill now states the merge boundary negatively: "complete end to end" adds merge, push to a shared branch, branch deletion, and cleanup, and nothing else grants those — not "fix", not the repository's usual flow, not an Issue, not an existing branch. It also says that failure to read a required rule does not erase it, and that the reference list is read before the act it governs.
...
- Every 1.8.0 behavior change is `UNVERIFIED`. Each needs a receipt from a real run: a delivery request that stops short of merging, a run whose report keeps an empty heading, and a session with qualifying `github.md` body evidence before its first write. A fourth session, re-read after it finished, narrows the first of those: it had qualifying `github.md` body evidence including the prohibition and merged and pushed regardless, so the receipt is a run that stops at the branch rather than one that merely has the reference body in context ([field audit](docs/research/2026-08-26/field-audit-2026-08-26.md), [ADR 0015](docs/decisions/0015-unconditional-invariants-live-in-the-root.md)).
