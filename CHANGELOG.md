...
- The package goes from 21 em dashes and 70 semicolons to none and one, the survivor being in the frontmatter description that `tests/test_package.py` pins. Sentences over forty words fall from 46 to 8 and paragraphs over a hundred words from 41 to 12, counting prose paragraphs and skipping headings and list items. Thirteen files over four hundred words carried no subheading and now none do. The text grew by 360 words, all of it scope lines, headings, and sentence splits, because this project has decided that method length is not the constraint.
