...
- Deterministic checks pass (47 tests). The root is 983 words of 1,000 and 6,352 bytes of 7,000; the largest reference is `model-routing.md` at 598 of 600, `long-work.md` next at 596, and the references total 3,897 of 4,000. Claude package validation and Claude isolated install pass locally; Codex package validation and Codex isolated install are `UNVERIFIED` here, as this machine's managed Codex policy rejects the local marketplace.
...
- The candidate was then reviewed through the same rung, in two lanes, and eleven findings came back, all of them acted on: the hook shape excluded `"`, `$`, backtick and backslash from the message but not `'`, which closes the outer `sh -c '...'` and runs a program the denylist never named; the builder guard checked that the brief's files were present when the defect is that their contents are stale, so it now requires the checked-out commit to be the base; `UNSAVED` as "nothing recorded it" also described a `DISMISSED` finding and now names an action still needed; the widened personal-path pattern reached into `/users/` routes inside documented URLs, which would block a release on innocent text; `isinstance(False, int)` is `True`, so a boolean metric still rendered as a zero; rejecting a second `result` event broke a valid streaming-input transcript, which emits one per turn; the reference gained a fifth reason to stop and README and how-it-works still said four; and three of the new tests compared the package with the allowlist rather than running the check, so they passed by construction. Each of those five is now mutated against as well.
