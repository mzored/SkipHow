...
the agent finds only in the kernel. Six sentences in the bug workflow and three references still told the run to record or
...
- `skiphow-bug`, `references/operations.md` (three sentences), `references/diagnosis.md`, and `references/integration.md` no longer restate the disposition 5.1.0 replaced: "record a separable problem", "record it as technical work", "preserve the finding … instead of silently widening the request", "otherwise preserve the evidence in the project's existing work system", "report the control", and a leftover branch's unconditional "finish, integrate, or record it". The sentences are rewritten rather than deleted. One drops its record exit, and the other five point at the kernel's disposition for a problem you find. Evidence: one private 5.1.0 change session, with the kernel in context whole, delivered part of an item already open in the project's tracker, commented on it, and reported the rest as still open; the owner had to start another session to have it done. The contradictions are a readable property of the text. Located by each file's opening sentence, that session had the bug workflow, `diagnosis`, `integration`, and `tracked-work` in context and not `operations`; which reading decided it is `UNVERIFIED`. The report the owner quoted came from a later session still on 5.0.0, whose "without widening the work" 5.1.0 had already replaced.
- `references/tracked-work.md` adds one sentence under "Closing what the tracker carries": what this session's delivery leaves open in an item, or a finished session's in one the request reaches, is a problem you found, and the open item alone does not dispose of it. It loads at the step where that session decided to keep the item open. A live session's remainder stays that session's, and an earlier session's reaches the run only through the request, so the tracker's backlog does not become the run's. A kernel clause covering problems already on record was refused. It would load on every request in a kernel six characters under its view limit, and it would invite loosely related backlog into a change request.
...
The Codex review round was blocked by the review service's usage limit, so cross-host review of this change is not claimed. An independent same-host review (Opus) raised three qualifying findings, all confirmed against the files and fixed. First, two more sentences carried the replaced disposition, one in `operations` under "Learn from failures" and one in `integration` on a leftover branch, and the records claimed none remained. Second, the records said the stale sentences were deleted and that each pointed at the kernel; they were rewritten, and one only drops its record exit. Third, "whichever session delivered it" reached an item a live concurrent session still holds, against `tracked-work`'s own rule and the kernel's custody, and reached any partly delivered item in the tracker; the sentence now names this session and a finished one, the latter only in an item the request reaches. A second round over the fixes raised two more, both in the records and both fixed: the decision index still stated the broad scope, and the decision history placed `integration` among sentences about a separable problem and in the session's context without saying how that was established.
...
- `SKILL.md`'s disposition of a found problem replaces "without widening the work" with the owner's ruling of 2026-09-19. A problem the change itself adds is still fixed before delivery. Read-only requests, iteration, protected actions, owner constraints, and a trusted project's narrower procedure are unchanged. Until a requested change is delivered, a problem outside it is reported, which keeps read-only requests and iteration writing nothing for it. Evidence: 15 host follow-up suggestions in 6 sessions carrying the kernel between 9 and 18 September 2026, four of them withdrawn as taken up in the same session, two at the owner's request. The disposition list had let an ephemeral card pass as recorded, while it handed a technical and priority decision back to the owner.
- `SKILL.md` makes room within its one-view limit by deleting method its references already carry at the act: the sentence on when delegation repays coordination, which `delegation` states in full under "Whether to delegate at all" and which the kernel still requires in context before any dispatch, and the confirm-fix-re-review clause of the final review, which `verification` states in full. "A material separable problem" left the tracking trigger because the disposition sentence now routes one to the tracker.
...
The Codex review round was blocked by the review service's usage limit, so cross-host review of this change is not claimed. An independent same-host review (Opus) raised three qualifying findings, all confirmed against the files and fixed. First, a tightening of the disposition sentence had dropped its "once delivered" scope, which let it contradict read-only requests and iteration and left reporting off the list of exits. Second, the CHANGELOG pointed the delegation criteria at the wrong section of `delegation`. Third, the suggestion counts could not be reproduced as first written; they were recounted with the method now stated. A second round confirmed the fixes and raised nothing further. It also confirmed that "delegate only where it repays coordination" could leave the kernel, because `delegation` carries the criteria and the kernel still requires that file before any dispatch.

The full local package gate passed all 391 tests under the pinned dependencies, and `git diff --check` passed. `scripts/check_hosts.py` passed Claude schema validation and a clean Claude install; the Codex validator was unavailable locally and pull-request CI supplies it for its revision. Invocation, activation, and continuity were not retested. Behavior under the new text is `UNVERIFIED` on both hosts, and no paid behavioral run was made.
...
| Implicit activation | UNVERIFIED |
| Continuity | UNVERIFIED |
| Behavioral suite | UNVERIFIED, [evidence](docs/evidence.md#510-found-problems-and-integration-cadence) |
...
- A checkout, branch, running service, or uncommitted change the run did not create is still never overwritten, reset, published, deleted, or quietly absorbed, and it is no longer left unaccounted either. Where the work brings the agent up against material shared state, the report says once what it appears to be, what can be read claiming it, and the one action that would settle it. Naming it is not authority to act on it: that action is taken only where the current request and the rules governing that act already allow it, and recommended otherwise while the work carries on. Observed defect: the owner reports sessions on their projects answering "that is not my work, I will not touch it, and I do not touch uncommitted files either", and the shipped text made that answer compliant. The kernel carried five prohibitions on unowned state and no counterpart duty. `references/integration.md` required reporting only for branches, checkouts and checkpoints, and only once that playbook had opened, and uncommitted work left by a session that is gone had no disposition at all.
- No technical choice reaches the owner, as a question, as a menu of options, or as permission to proceed on engineering grounds. Where a technical uncertainty is reversible, the agent takes the option it would defend and says which it took. What reaches the owner instead is a recommendation: the decision already made, or the next action the agent would take on their word, in the consequence it carries for them. Observed defect: no shipped sentence forbade the technical question itself. `references/operations.md` forbade transferring ordinary technical review back to the owner, `references/product.md` governed how a question that does reach them is put, and the kernel already allocated those decisions to the agent without ever saying they may not be handed back. The corpus forbids technical approval, tracker interviews and test-selection questions in five cases, each resting on that allocation rather than on a prohibition.
- A running result shown to the owner names the branch or revision it serves and the address serving it. Observed defect: several checkouts of one project answer on the same default address, nothing in the package asked for the branch to be named beside the preview, and the owner reports concluding from a stale stand that an agent had removed screens it never touched.

### Behavioral coverage

- Naming the branch an earlier run left behind moves from permitted to required in the candidate arms of the existing stale-branch case, and its acceptance now covers what can be read claiming the branch and the action that would settle it.
- The preview iteration case gains an event for the branch or revision a shown result serves, and for the address where it runs at one.
- The foreign uncommitted-work case is unchanged. Naming that work and saying it was left alone is already required of every arm there, and asking the owner what to do instead of finishing is already forbidden, so this release adds nothing that fixture can distinguish.
...
- ``codebase-design`` now also opens on an existing structure the owner has asked to improve. That request is neither an interface nor a module boundary, so it matched the old trigger only by accident, and no other method reached it: `prioritization` fires on candidates already recorded and `diagnosing-bugs` on defects.
...
- The claim rule moves out of ``advancing-tracked-work`` into `tracked-work`, so a single named item reaches it too. Its trigger fires only when the owner asks to carry on with the backlog, which left one item worked without a claim at all. A claim that loses now means another session holds the item rather than that the claim is worth retrying.
- ``finishing-a-branch`` now closes the item the work was tracked under, confirming a tracker-performed closure rather than assuming it happened. It never mentioned the tracker before, so nothing connected an integrated change to the item that asked for it.
...
- ``project-setup`` records the calls a tracker needs for claiming, linking, recording a dependency, and closing, wherever they are not obvious from its own interface, and a recorded call that stops working joins the refresh conditions. Same note, same file, no new format.
- ``decomposition``, ``prioritization`` and ``continuity`` point at `tracked-work` instead of `intake`.
