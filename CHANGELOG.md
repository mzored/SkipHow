...
- Every per-lane rule in `execution-health` worked as written in those runs. No numeric limit was added; a two-hour rule and a one-item-per-session rule were both refused again, see `docs/decisions.md`.
- Behavior on the new wording is `UNVERIFIED` until a paired receipt or the owner's next long campaign; see `docs/evidence.md`.
