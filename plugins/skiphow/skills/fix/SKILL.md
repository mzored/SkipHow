...
When an accepted tracked item or repository policy requires GitHub lifecycle work, read `../github-task/SKILL.md` only after that need is established. For an independent material finding, decide `PERSISTED` first and only then load the owning tracker adapter to search for a duplicate and save it. The adapter does not reclassify the repair or choose its engineering rigor.
