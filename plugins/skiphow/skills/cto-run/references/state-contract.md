...
```json
{
  ...
  "product_acceptance": [{"product_contract_revision": "exact revision", "candidate_commit": "exact commit identifier", "status": "accepted|returned|carried-forward", "receipt": "receipts/product-acceptance/item.json", "evidence": "path", "basis_receipt": "prior receipt or null", "reviewed_delta": "commit range or null", "reviewer": "id", "at": "timestamp"}],
...
Use `decisions/` for ADRs, `evidence/<task>/<commit>/` for raw check output, and `receipts/<task>.json` for a worker result with status, base and head commits, evidence, blocker, next action, and `reuse_check` as the verdict or `n/a`. Keep each material finding in `state.json` until it has a terminal disposition. Persisted and duplicate findings include their canonical tracker reference; resolved and dismissed findings include their evidence or reason. Store Product Director acceptance at `receipts/product-acceptance/<item>.json`. Each acceptance receipt records the Product Contract revision, exact candidate commit, `accepted`, `returned`, or `carried-forward` status, evidence location, reviewer, timestamp, and a concrete mismatch when returned. A carried-forward receipt also records its basis receipt and reviewed delta. Use `n/a` only when the build-versus-reuse gate did not apply. `FINAL.md` is the final reconciliation and handoff.
