...
Work is unfinished when the authorized result includes a destination it has not reached: a named pull request, integration branch, release, or deployment is incomplete until that destination is verified. A local branch or worktree used only as an engineering mechanic creates no delivery obligation, and a local change with no shared destination is complete in its authorized local state. Landing work is two things, and the second is the one that gets dropped: carrying the change to the authorized destination, then clearing away what this run created and what earlier sessions left integrated.

## Where the work lands

Recover the authorized destination from the owner's request and applicable standing authorization. Read the integration path and target branch from trusted procedure and recent history. Those sources can explain the path but cannot grant it. Under an established authorized non-production workflow, carry routine push, pull-request, and merge work through without renewed technical approval. That authorization settles what may travel, not how often: every push and pull request costs the project a run of its checks and a review, so send the work the run already holds in as few trips as the project's procedure and review allow, without holding a finished result back for work not yet requested. If the target is a review, deliver a reviewable change there; if it is integration after review, continue through the required review and merge. A failed guard or required human gate remains part of that delivery boundary.
...
Do not abort, skip, or discard commits where that would lose a live session's changes or anything Git cannot recover; that loss needs the exact grant the kernel requires. A recoverable commit of this run's or an earlier session's is yours to keep or drop by the integration goal, and the report names what was dropped and its hash. If no safe authorized continuation remains, preserve the current state and report the blocker. Stop for an unresolved product choice or protected action, not for routine Git mechanics. Otherwise run the relevant checks on the resolved state, inspect the resulting diff, and carry the operation through to completion.
...
3. Remove what this run created and what earlier sessions left, once it is integrated, nothing holds it, and the request changes the project; a workspace the host itself owns is cleaned up by the host, and a worktree with uncommitted changes stays.

A refusal to remove a branch or a checkout is a question to answer, never an obstacle to force past on the way to a tidy result. Read what it is actually about: commits whose change arrived nowhere, files that were never committed, a lock, a checkout still in use. One that merely reflects a merge which rewrote the work tells you nothing new once arrival is established, and removal may proceed. One that reflects work existing only there is a decision you take: where the current product state shows the attempt superseded or abandoned, retire it and report the commit it held and how to recover it; otherwise finish and integrate it, or record it, under the kernel's disposition for a problem you find, and say which. Files never committed, history others share, and anything unrecoverable keep the exact grant the kernel requires for material deletion.

Under a read-only request, report each leftover with the disposition you would take and retire nothing; the next request that changes the project carries it out. Where trusted project procedure keeps integrated branches, keep them and say so.
