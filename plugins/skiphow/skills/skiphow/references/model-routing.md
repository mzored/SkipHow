...
Claude Code: the plugin ships `scout`, `builder`, and `reviewer` under `agents/`; invoke them as `skiphow:<role>`. Its isolated builder may branch from the remote default rather than the brief's base. Use it only when the created worktree matches the exact base; otherwise the root builds in an exact-base isolated lane or delegates through another lane whose worktree and branch it can bind safely, without asking the owner. The reviewer runs on the session model. If the host substitutes a model, treat the route as inherited and say so.
