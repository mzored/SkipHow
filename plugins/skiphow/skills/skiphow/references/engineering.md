...
When Git is already in a conflicted merge or rebase, recover both sides' intent from the merge base, commits, Issue, pull request, repository decisions, and tests before editing. Conflict markers show overlapping text, not every semantic conflict, so also check renamed symbols, moved files, schemas, generated artifacts, and tests outside the markers. Resolve the combined intent when the changes are compatible; otherwise follow the owner request and accepted decisions, never the newer, larger, or easiest side. Ask only when the incompatible intents require a material product choice evidence cannot settle.

Complete, abort, or restart only an operation whose worktree, branch, initiating intent, and expected source and target heads are recorded as owned by this run. Isolate away from a foreign or ambiguous operation and mark that lane `BLOCKED` rather than touching it. Prefer completing an owned operation; abort and restart only when it cannot safely express the intended integration and every unique change has first been preserved and identified. Never reset or discard foreign work. Stage only resolved paths, run checks for both intents, finish with ordinary merge or rebase commands and hooks, then include the result in exact aggregate review. A conflict is work to complete, not by itself a blocker.
