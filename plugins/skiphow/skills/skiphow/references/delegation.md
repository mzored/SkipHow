...
Naming a model or effort in your own message is not setting it. Set the host's per-delegate control where it exposes one and read the effective setting back where the host reveals it. Report an unavailable control or hidden effective setting honestly, without claiming an unverified route was applied. Where only inheritance is available, account for that constraint when deciding whether delegation still pays.
