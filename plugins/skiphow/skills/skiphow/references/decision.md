...
Routine reasoning stays in the working notes. Write a durable record when the decision is consequential, disputed, regulated, or must outlive the session, and always when data crosses a private, internal, or public boundary or a change supersedes an existing durable decision. Update the owning record rather than creating a competing one. A code comment or test alone is not the product record.

A durable record names the outcome and its users, the evidence, the decision and the rejected alternatives, what changed in behavior, the reversal condition, and the revalidation trigger. Keep libraries, schemas, file paths, and test strategy out of it unless they create a real product tradeoff.

## Acceptance

For a material contract change, a regulated or high-impact flow, or an explicit repository gate, name the user-visible scenarios that need acceptance and the evidence for each, then recheck the ones whose journey, output, error state, accessibility, or privacy behavior changed. A data-boundary change includes disclosure and withdrawal scenarios. An ordinary change gets no acceptance ceremony.
