...
Read the integration path and the branch this work returns to off the project's own recent history rather than asking for them. Where the project's history shows work merged straight into an integration branch, do that. Where work lands through review instead, the change reaches that review in the state it should be reviewed in, and this branch is finished there. Merging past a gate the project keeps is not finishing faster.

Finishing one branch that way ends that branch and not the session, which is free to carry on with work that does not wait on it. That case leaves the branch's own end later than that of the run which built it, which is what the late collection below exists for. Writing to a shared branch is still shared delivery and follows [delivery](delivery.md).
