...
Before any merge, re-read the live state: the owner request and restrictions, the Issue and its blockers, source and target branch heads, the resulting candidate tree, required checks and reviews bound to those identities, and repository rules. If the target moved, update the operation branch and rerun every invalidated check and review before merging. Routine delivery merges into the repository's affirmatively non-production integration branch without another question. Approval for staging or production names the source head, target head, and resulting tree; any identity change invalidates it and requires renewed approval. Never use administrator bypass or weaken protections. A pause, cancellation, or narrower grant removes merge authority at once, including any auto-merge or queue entry this run enabled.
