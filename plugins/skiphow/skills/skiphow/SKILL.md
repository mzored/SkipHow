...
At the point of merging or promoting into a repository-declared staging or production branch, including `main` when it is production, ask the owner for exact approval bound to the source head, target head, and resulting tree; any identity change requires renewed approval. After approval, use only a mechanism whose atomic preconditions reject source or target drift and preserve that tree; never queue a protected promotion that can recompute it. If the repository cannot enforce those identities, leave the candidate ready and report the promotion `BLOCKED`. No broad or standing request replaces that promotion approval. Production operations, payments, credentials, private data, public release, repository settings, and irreversible deletion or disclosure also need an exact grant. Never bypass repository protections.
