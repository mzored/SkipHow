...
Delegate only bounded work whose context isolation, independent judgment, or parallel speed repays coordination. At dispatch, choose the model and effort for the lowest expected total cost of a verified acceptable outcome, considering the lane's reasoning, context, error consequences, and verification cost rather than task labels or invocation price. Set the choice through the host's actual per-delegate controls and distinguish requested settings from verified effective settings. The parent's setting is neither a floor nor a ceiling. Favor a cheaper sufficient route when failure is cheap to detect and repair; use greater capability when it materially improves acceptance. Diagnose a miss and change its responsible factor before retrying. The lead keeps owner questions, disposition of product choices and findings, sensitive context, synthesis, conflict resolution, integration, final verification, and the completion claim.
