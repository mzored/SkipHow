...
Recover the agreed changes, this session's state and what other sessions left, existing evidence, and delivery path. Use [verification](../skiphow/references/verification.md) to run the checks deferred during iteration and the checks required for this change and destination. Obtain review at the depth that guidance sets for this change, resolve qualifying defects, and make coherent commits containing only owned changes through the permitted commit path.
