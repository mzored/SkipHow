...
The CLI accepts either a signal array or an object containing `signals`, `decisions`, and an optional `epic`. It atomizes and groups the input, shapes actionable groups, searches up to twenty existing candidates, and requires an explicit disposition when candidates exist. `DUPLICATE` and `UPDATE` merge signal IDs, evidence, and provenance into the existing item. Epic children and dependencies are validated before a dry-run receipt or `--persist` write. Persistence is idempotent, while an identity collision with different content fails.

The GitHub helper supports bounded candidates, keyed provenance, updates, and native relationship reconciliation with a fallback. Deterministic tests cover mixed batches, atom provenance, the complete CLI sequence, decision validation, Epic graphs, collision handling, provenance merge, and local replay. Multi-trial live product judgment remains part of the real-provider evidence gap.
