...
- Ask at the point of promotion into a repository-declared staging or production branch, including `main` when it is production. Bind approval to source head and target branch, and show the current target head and resulting tree. Target movement invalidates evidence, not the product approval: update and re-review the candidate, and ask again only when source content, target branch, or product or rollout meaning changes. Pin the source atomically where the host supports it, re-read the target immediately before merge, and verify the actual result. Do not ask for worktrees, branches, ordinary commits, required Issues and pull requests, review loops, conflict resolution, routine integration, Issue closure, or safe owned cleanup.
- Re-read repository instructions, Git identity, and observable active host tasks before mutation. When no task inventory exists, absence remains unknown: a bounded single-writer run may retain a clean or attributed checkout under continuous identity guards, while parallel, unattended, recovered, or ambiguous work uses fresh isolation. Re-size after every owner turn. Independently landable additions become explicit queue units.
...
- Every independently landable unit has one root operation branch. Every builder commits its owned delta through the repository's ordinary commit path and hooks; the root verifies and integrates those commits into the unit branch. The unit is reviewed and delivered independently, so a blocked unit does not hold ready work. Routine delivery is complete when the reviewed unit reaches the proven non-production integration target through the repository's normal path, whether that path uses a merge or a direct commit and push. A direct push fetches and re-reads the remote target, requires the candidate to descend from that exact head, and rejects force or non-fast-forward updates.
- Bind checks and review to target head, source head, and resulting committed tree after hooks. Once a unit's commits are integrated and the target is current, run fresh affected checks and independent review on that exact aggregate candidate. Target movement, integration, hook changes, conflict resolution, or later edits invalidate applicable evidence. Fixes are re-reviewed. After successful queue units land, run fresh cross-item checks on the current target and turn a confirmed interaction into a repair unit.
...
### Review builder commits but not the unit aggregate

Clean integration can still create semantic interactions, and target movement changes the merge result. Each independently landable unit needs aggregate evidence before it enters the target; a final cross-item pass covers the queue state without making one blocked unit hold all ready work.
