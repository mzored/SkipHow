...
- Ask at the point of promotion into a repository-declared staging or production branch, including `main` when it is production. Bind approval to source head, target head, and resulting tree; identity changes require renewed approval. After approval, use only a mechanism whose atomic preconditions reject source or target drift and preserve that tree; a queue that can recompute the candidate is not such a mechanism. If repository protections offer no identity-safe path, leave the candidate ready and report the promotion blocked. Do not ask for worktrees, branches, ordinary commits, required Issues and pull requests, review loops, conflict resolution, routine integration, Issue closure, or safe owned cleanup.
