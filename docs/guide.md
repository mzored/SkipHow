...
The instructions require SkipHow to preserve unrelated work: a checkout, branch or running service it did not create is shared, and the uncommitted changes in it are somebody's. Preserving it is not the same as ignoring it. From 4.8.0 the report also names the shared state the work brought it up against, says what it appears to be, what it can read claiming it, and the one action that would settle it, so a branch an earlier session finished and never merged stops being invisible. Naming it is not permission to act on it: it takes that action only where your current request and the rules covering that action already allow it, and otherwise recommends it and carries on. It never overwrites, resets, publishes, deletes, or quietly absorbs any of it. One run on record broke that and destroyed a peer session's uncommitted work, so if you keep two sessions on one checkout, read [current evidence](evidence.md) before you rely on it. A dirty checkout is a reason not to commit when the owned change cannot be separated safely, and it weakens verification only when it makes the evidence unreliable.
