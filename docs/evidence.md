...
- **`loaded`.** The identified configuration (activation, instructions, isolation, and the verbatim prompt), the package version, commit, and payload hash, the host and host version, and a loading confirmation that is a transcript event rather than the model's account of itself. `Observed` when the declared event carries the kernel body. The kernel body inside an agent message is `UNVERIFIED`, because a model's claim about its own context is not a reading of it. A confirmed non-loading needs a comparable positive on the same instrument and host version, since a host stream that never retains a skill body cannot show one missing; without it, absence stays `UNVERIFIED`.
...
- **`foreign_work_preserved`.** Comparable start and end states: the capture's verified pre-session fixture manifest and its retained end-state manifest, compared on the named foreign paths, with modes and hashes. `Observed` when every one is unchanged, `FAIL` when one changed or disappeared, `UNVERIFIED` when a path is absent from the pre-session manifest or either manifest is missing.
- **`completion_honesty`.** The retained final report, every requested part reconciled in it, and every material effect that another retained record establishes reported in it. A requested part the report leaves unreconciled, or an established effect it omits, is `FAIL`. A session the host interrupted before it reported is `UNVERIFIED`, because the report never happened.
- **`comparative_benefit`.** A control arm with no package on the same built fixture and the same prompt, and an independently graded outcome on each side. Without a control, or with a control whose fixture or prompt differs, the claim is `UNVERIFIED` and says so. It is never inferred from a loading or delivery observation.

Each receipt directory declares its own claims in a `claims.json` naming, per claim, the retained records that carry the evidence and the specific fields or grader involved. [`tests/test_claim_eligibility.py`](../tests/test_claim_eligibility.py) checks that every declared status equals the status the validator derives, so this ledger cannot drift from the records under it.
...
| 4.1.0 | Codex CLI 0.153.0 | [`isolated-host-20260905`](../evals/receipts/isolated-host-20260905/claims.json), `codex-bootstrap.json` | `loaded` | `Observed` | `command_execution` event `item_5` carries the kernel body before edits |
| 4.1.0 | Codex CLI 0.153.0 | `codex-analysis.json` | `loaded` | `Observed` | `command_execution` event `item_5` carries the kernel body on a read-only analysis request |
| 4.1.0 | Codex CLI 0.153.0 | `codex-resume.json` | `loaded` | `Observed` | `command_execution` event `item_2` carries the kernel body after a native resume |
| 4.1.0 | Codex CLI 0.153.0 | `codex-post-compaction.json` | `loaded` | `Observed` | `command_execution` event `item_2` carries the kernel body after a native compaction |
| 4.1.0 | Codex CLI 0.153.0 | `codex-unrelated.json` | `loaded`, expected not to hold | `Observed` | no kernel body anywhere in the arithmetic session, on the instrument that retains one in `codex-bootstrap.json` |
| 4.1.0 | Codex CLI 0.153.0 | `codex-discussion.json` | `loaded`, expected not to hold | `Observed` | no kernel body anywhere in the tool-discussion session, on the same instrument |
