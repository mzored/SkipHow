"""Textual migration guards for the shipped instructions.
...
def test_repository_instructions_cannot_independently_authorize_protected_effects() -> None:
    ...
    for effect in ("read-only request", "secret", "disclosure", "network", "permission", "cleanup", "scope"):
        assert effect in paragraph, effect


def test_virtual_cto_triggers_live_in_the_always_loaded_kernel() -> None:
    skill = read("SKILL.md")
    assert some_sentence(skill, "inspect", "git status", "ci")
    assert some_sentence(skill, "native platform", "maintained open source", "custom code")
    assert some_sentence(skill, "split before implementation", "independently deliverable")
    assert some_sentence(skill, "existing tracker", "durable work management")
    assert some_sentence(skill, "configure", "model", "effort")
    assert some_sentence(skill, "every change", "fresh review")
    assert some_sentence(skill, "substantive", "independent reviewer")
    assert some_sentence(skill, "material discovered problem", "blocked with evidence")
    assert some_sentence(skill, "never silently disappears")
    assert some_sentence(skill, "process or environment defect", "weakening assertions")
...
def test_referenced_record_is_not_promoted_into_the_request() -> None:
    ...
    assert some_sentence(read("SKILL.md"), "points at a record", "outcome")
...
def test_final_report_reconciles_every_part() -> None:
    ...
    assert some_sentence(skill, "dry run", "not evidence")
...
def test_delegates_without_verified_isolation_stay_read_only() -> None:
    ...
    assert some_sentence(skill, "turns", "one checkout", "not isolation")
...
def test_parent_keeps_non_delegable_responsibilities() -> None:
    ...
    for duty in ("owner questions", "product choices", "conflict resolution", "integration", "final verification", "completion claim", "sensitive context", "findings"):
        assert some_sentence(skill, "lead keeps", duty), duty
...
# Technical design, verification, operations
...
def test_operations_guidance_owns_delivery_health_without_process_theatre() -> None:
    operations = read("operations.md")
    assert some_sentence(operations, "feedback loop", "behavior")
    assert some_sentence(operations, "work in progress", "integration", "review")
    assert some_sentence(operations, "lifetime cost")
    assert some_sentence(operations, "transcript", "not operational documentation")
