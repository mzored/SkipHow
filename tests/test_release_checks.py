    ...
    *,
    ...
    def checked(command, **kwargs):
        ...
        if command[1:3] == ["plugin", "list"]:
            ...
            if host == "codex":
                # Codex reports the marketplace source here. It is deliberately not
                # the installed payload path and must never be used as one.
...
def test_codex_inventory_requires_explicit_installed_boolean() -> None:
    ...
    # Claude's current inventory schema does not require this Codex-only field.
...
def test_isolated_install_still_requires_exact_payload_bytes(host: str) -> None:
    ...
    def corrupting_checked(command, **kwargs):
        ...
        if list(command)[1:3] == ["plugin", "list"]:
            home = Path(kwargs["env"]["CODEX_HOME" if host == "codex" else "CLAUDE_CONFIG_DIR"])
            (home / "installed/skiphow/VERSION").write_text("wrong\n", encoding="utf-8")
